...
class Apply < ActiveRecord::Base
  ...
  def self.statuses
    Apply.unsubmitted_statuses | Apply.not_ready_statuses | Apply.ready_statuses | ['accepted'] | Apply.not_going_statuses
  end
  ...
  def staff_reference
    get_reference("Staff Reference")
  end
  
  def discipler_reference
    get_reference("Discipler Reference")
  end
  
  def roommate_reference
    get_reference("Roommate Reference")
  end
  ...
  def get_reference(title)
    references.each do |r|
      return r if r.sleeve_sheet.title.downcase == title.downcase
    end
  ...
  def reference_answer_sheets
    r_sheets = Array.new()
    self.apply_sheets.find(:all, :include => :sleeve_sheet, :conditions => ["#{SleeveSheet.table_name}.assign_to = ?", 'reference']).each do |s|
  ...
  def find_or_create_reference_answer_sheet(sleeve_sheet)
    answer_sheet = nil
