...
module FileColumn # :nodoc:
  ...
  def self.init_options(defaults, model, attr)
    ...
    model_dir = File.join(options[:root_path], model)
    if options[:svn] and !File.exists?(model_dir)
      FileUtils.mkpath(model_dir)
      system("svn add #{model_dir}") 
      system("svn commit #{options[:root_path]} -m 'added #{model_dir}'")
    end
    ...
    [:store_dir, :tmp_base_dir].each do |dir_sym|
      if options[dir_sym].is_a?(String) and !File.exists?(options[dir_sym])
        ...
        system("svn add #{options[dir_sym]}") if options[:svn]
        system("svn commit #{model_dir} -m 'added #{options[dir_sym]}'") if options[:svn]
      end
    end
    ...
    def store_dir
      if options[:store_dir].is_a? Symbol
        ...
        unless File.exists?(dir)
          ...
          system('svn add ', dir) if options[:svn]
        end
        dir
      else 
        options[:store_dir]
  ...
  class TempUploadedFile < RealUploadedFile # :nodoc:
    ...
    def delete_files
      FileUtils.rm_rf(File.join(tmp_base_dir, @tmp_dir)) unless options[:svn]
      system('svn remove --force ', @tmp_dir) if options[:svn]
    end
  ...
  class PermanentUploadedFile < RealUploadedFile # :nodoc:
    ...
    def move_from(local_dir, just_uploaded)
      ...
      FileUtils.rm_rf @dir unless  options[:svn]
      system("svn remove --force #{@dir}") if options[:svn]
      system("svn commit #{store_dir} -m \"removed file(s)\"") if options[:svn]

      FileUtils.mv local_dir, @dir
      system("svn add #{store_dir}") if options[:svn] # this will only matter the first time this model/attribute is used
      system("svn add #{@dir}") if options[:svn] 
      system("svn commit #{store_dir} -m \"added file(s)\"") if options[:svn]
