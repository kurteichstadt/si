...
class PaymentsControllerTest < ActionController::TestCase
 ...
 def setup
  ...
  def setup_payment
    cas_login
    @payment = create(:staff_payment, :answer_sheet => @apply)
  end
  ...
  test "bad credit card" do
    xhr :post, :create, :application_id => @apply, "payment"=>{"city"=>"Villa Park", "address"=>"123 Somewhere St", "zip"=>"60181", "card_type"=>"visa", "security_code"=>"123", "expiration_year"=> Time.now.year, "card_number"=>"4111111111111112", "expiration_month"=> Time.now.month + 1, "last_name"=>"Joe", "state"=>"CA", "first_name"=>"GI", "payment_type"=>"Credit Card"}
    assert(payment = assigns(:payment))
  ...
  test "double paying" do
    create(:email_template)
     @apply.payments.create(:payment_type => 'Staff', :payment_account_no => '000559826', :staff_first => 'John', :staff_last => 'Doe')
    xhr :post, :create, :application_id => @apply, :payment => {:payment_type => 'Staff', :payment_account_no => '000559826', :staff_first => 'John', :staff_last => 'Doe'}
  ...
  test "mail payment" do
    create(:email_template)
    xhr :post, :create, :application_id => @apply, :payment => {:payment_type => 'Mail'}
    assert_response :success, @response.body
  end
  ...
  test "edit non-Staff payment" do
    cas_login
    @payment = create(:payment, :answer_sheet => @apply)
    get :edit, :id => @payment, :application_id => @apply
  ...
  test "destroy" do
    ...
    assert_difference "Fe::Payment.count", -1 do
      xhr :delete, :destroy, :id => @payment, :application_id => @apply
    end
    assert_response :success, @response.body
  end
